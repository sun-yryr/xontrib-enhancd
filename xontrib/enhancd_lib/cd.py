...
def before_cd():
    # OLDPWD の更新
    x_env["ENHANCD_OLDPWD"] = str(Path().resolve())

def after_cd():
    history.update_history()
    if x_env.get("ENHANCD_HOOK_AFTER_CD") is not None:
        ENHANCD_HOOK_AFTER_CD()

def builtin_cd(args):
    before_cd()
    x_env["DEFAULT_CD"]([args])
    after_cd()
